...
/client/proc/create_dungeon()
	...
	var/dungeon_type
	if(alert("What type of dungeon would you like to make?",,"Winding", "BSP") == "Winding")
		dungeon_type = /datum/random_map/dungeon/winding_dungeon
		room_types = typesof(/datum/room_theme)
	else
		dungeon_type = /datum/random_map/dungeon/bsp_dungeon
		room_types = typesof(/decl/room_constructor/dungeon)

	var/list/vars = list()

	vars["limit_x"] = input("What is the width of your map?") as num
	vars["limit_y"] = input("What is the height of your map?") as num
	if(dungeon_type == /datum/random_map/dungeon/winding_dungeon)
	...
	if(alert("Would you like to modify generational multipliers?",,"No","Yes") == "Yes")
		var/percent
		if(dungeon_type == /datum/random_map/dungeon/winding_dungeon)
			percent = input("What is the percent multiplier of how many features are generated? Default is 2 (WARNING HIGH NUMBERS MAY CAUSE LAG)") as num
	...
	var/mode = "room_theme"
	while(1)
		if(mode == "room_theme" && dungeon_type == /datum/random_map/dungeon/winding_dungeon)
	...
	new dungeon_type(null,usr.x, usr.y, usr.z, variable_list = vars)